...
from llm import is_key_dead, APIKeyDeadError
...
def _fail_job_if_key_dead(job: dict, api_key: str) -> bool:
    ...
    if is_key_dead(api_key) and job["status"] == "running":
        job["status"] = "failed"
        job["error"] = "OpenAI API key suspended or quota exhausted. Please check your API key and billing."
        job["current_step"] = "FAILED — API key suspended"
        ts = time.strftime("%H:%M:%S")
        job["log"].append(f"[{ts}] ABORTED — API key is dead")
        logger.error(f"Job {job['job_id'][:8]} aborted — API key is dead")
        return True
    return False
...
async def _sos_dispatcher(worker_id: int):
    ...
    while True:
        ...
        async def _on_entity_result(entity_dict, sos_result):
            ...
            await _company_queue.put((job_id, entity_idx, api_key))
            job["company_total"] += 1

            # Queue person search for each human officer/agent
            for person in people:
                await _person_queue.put((job_id, entity_idx, person, api_key))
                job["person_total"] += 1
...
async def process_fdd(
    ...
    openai_key_override: str = Form(default=""),
    ...
    """
    openai_key = openai_key_override or OPENAI_API_KEY
    bu_key = browser_use_key_override or BROWSER_USE_API_KEY
    if not openai_key:
        raise HTTPException(400, "OPENAI_API_KEY is required for PDF extraction")
    if not bu_key:
        raise HTTPException(400, "BROWSER_USE_API_KEY is required for browser agents")
    ...
    # ── Extract entities (uses OpenAI for PDF/XLSX parsing) ──
    try:
        if ext == ".pdf":
            entities = await extract_entities_from_pdf(file_bytes, openai_key)
        else:
            entities = await extract_entities_from_xlsx(file_bytes, openai_key)
    ...
    for st, batch in state_batches.items():
        await _sos_queue.put((job_id, st, batch, openai_key))
